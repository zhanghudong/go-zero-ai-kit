...
- 入口先做必要前置检查，再进入核心流程。
- 核心流程采用分步注释组织，形如 `// ========== 步骤1：... ==========`。
- 外部调用 + 数据映射只有在形成稳定语义或复用价值时才拆成私有方法（如 `getXxxDetail`）；仅为封装一次性主流程步骤时不得拆。
...
- 幂等处理必须建立在唯一约束之上，可通过唯一字段查询（如 `payment_no`）或唯一索引写入后结合 `dberr.IsDuplicateEntry` 收口；不得依赖无唯一约束字段做伪幂等。
