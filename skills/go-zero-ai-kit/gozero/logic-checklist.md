...
- DB 查询失败必须日志+错误包装返回（错误类型按项目规范选择）

6. 时间处理
- 统一使用项目公共时间工具包（如 `timeutil`）进行解析/格式化
...
9. 业务字段补充
- 操作人信息需查询用户模型并区分 `ErrNotFound` 与系统错误，失败必须返回错误

10. 并发与租户
- 状态更新必须使用条件更新（如 `WHERE id=? AND status=pending`），并检查影响行数
- 租户隔离必须强校验（不可因请求缺字段而跳过）
